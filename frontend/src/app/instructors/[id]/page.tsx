...
const teachers = [
  {
    ...
    experience: '8年',
    introduction:
      'こんにちは！アメリカ出身のサラです。日本に住んで5年になります。皆さんが楽しく英語を学べるよう、一人ひとりに合わせたレッスンを心がけています。大学では言語学を専攻し、卒業後は企業での英語研修も担当していました。',
    languages: ['英語（ネイティブ）', '日本語（上級）', 'スペイン語（中級）'],
    certifications: ['TESOL', 'TOEIC 990点', 'Cambridge CELTA'],
    hobbies: ['料理', '映画鑑賞', '旅行', 'ヨガ'],
    teachingStyle:
      'フレンドリーで楽しい雰囲気を大切にし、間違いを恐れずに話せる環境作りを心がけています。実際の場面を想定したロールプレイングを多く取り入れ、実用的な英語力の向上を目指します。',
    featured: true,
    rating: 4.9,
    reviewCount: 127,
  ...
  },
  {
    ...
    experience: '6年',
    introduction:
      'イギリス出身のジェームスです。正しい発音と自然な表現を身につけたい方、ぜひ一緒に学びましょう！特に発音矯正とプレゼンテーション指導を得意としています。',
    ...
    teachingStyle:
      '発音とイントネーションに重点を置き、実践的なコミュニケーション能力の向上を目指します。音楽的なアプローチで楽しく発音を学べるよう工夫しています。',
    featured: false,
    rating: 4.8,
    reviewCount: 89,
              ...
              <div>
                ...
                <div className="flex gap-4">
                  ...
                  <Button
                    ...
                    asChild
                  >
                    <Link href="/contact">無料体験レッスン</Link>
                  </Button>
                </div>
              </div>
                      ...
                      <ul className="space-y-2">
                        {teacher.workExperience.map((exp, idx) => (
                          <li key={idx} className="flex items-start">
                            <svg
                              className="mr-3 mt-0.5 h-5 w-5 flex-shrink-0 text-primary-600"
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                              <path
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {teacher.achievements.map((achievement, idx) => (
                          <li key={idx} className="flex items-start">
                            <svg
                              className="mr-3 mt-0.5 h-5 w-5 flex-shrink-0 text-yellow-500"
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  ...
                  <div className="space-y-2">
                    {formatSchedule(teacher.schedule).map((item, idx) => (
                      ...
                      >
                        ...
                        <span
                          className={`text-sm ${item.time === '休み' ? 'text-red-500' : 'text-gray-600'}`}
                        >
                          {item.time}
                        </span>
                      </div>
...
// メタデータ生成
export async function generateMetadata({ params }: TeacherDetailPageProps) {
  // Next.js 14/15 互換性のための処理
  const resolvedParams = params instanceof Promise ? await params : params;
  const { id } = resolvedParams;
  const teacher = teachers.find(t => t.id === id);
  ...
  return generatePageMetadata(
    `${teacher.name}（${teacher.nameJapanese}）`,
    teacher.introduction,
